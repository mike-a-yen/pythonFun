...
list_average = float(sum(list_of_numbers))/len(list_of_numbers)
...
square_dict = {str(i):i**2 for i in range(0,100)}
...
representation_dict = {k:[x,y,z] for (k,[x,y,z]) in list(zip(range(1000),[[x,y,z] for x in digits for y in digits for z in digits])) }
...
listdict2dict = { name:sal for (name,sal) in list(zip(names,[id2salary[id] for id in id2salary.keys()])) }
